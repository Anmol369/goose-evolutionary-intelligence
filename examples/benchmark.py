...
def run_single_benchmark(ecosystem: MinimalViableEcosystem, request_data: Dict[str, Any], trace: bool = False) -> Dict[str, Any]:
    ...
    ecosystem_verification_time = traditional_time * (1 - response.get('reality_alignment', 0.7))
...
def run_benchmarks(ecosystem: MinimalViableEcosystem = None, runs: int = 1, seed: int = None, trace: bool = False) -> Dict[str, Any]:
    ...
    if trace:
        print(f"🏃 Running {runs} benchmark run(s) with {len(BENCHMARK_REQUESTS)} requests each")
    ...
    for run_num in range(runs):
        ...
        for i, request_data in enumerate(requests):
            if trace:
                print(f"  [{i+1:2d}/{len(requests)}] {request_data['category']:<15} | {request_data['complexity']:<6}")
        ...
        if trace:
            ...
            print(f"     Mean reduction: {run_summary['mean_reduction_percentage']:.1f}%")
    ...
    for category, results in by_category.items():
        ...
        category_stats[category] = {
            ...
            "stdev_reduction": statistics.stdev(reductions) if len(reductions) > 1 else 0.0,
    ...
    final_results = {
        ...
        "overall_statistics": {
            ...
            "stdev_verification_time_reduction": statistics.stdev(all_reductions) if len(all_reductions) > 1 else 0.0,
        ...
        "detailed_results": all_results if trace else []  # Include details only if tracing
    }
...
def main():
    ...
    try:
        ...
        print("=" * 50)
        print(f"Total requests processed: {results['benchmark_metadata']['total_requests']}")
        print(f"Mean verification-time reduction: {results['overall_statistics']['mean_verification_time_reduction']:.1f}% ± {results['overall_statistics']['stdev_verification_time_reduction']:.1f}%")
        print(f"Total time saved: {results['overall_statistics']['total_time_saved_seconds']:.1f} seconds")
        print(f"Patterns learned: {results['benchmark_metadata']['final_patterns_learned']}")
        
        print("\n📈 CATEGORY BREAKDOWN:")
        for category, stats in results['category_breakdown'].items():
            print(f"  {category:<15}: {stats['mean_reduction']:6.1f}% ± {stats['stdev_reduction']:5.1f}% ({stats['count']} requests)")
